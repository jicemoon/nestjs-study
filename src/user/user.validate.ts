...
import { NameReg, PasswordReg } from '../configs/vertifyRegs';
...
const name: joi.StringSchema = joi
  .string()
  .min(3)
  .max(200)
  .regex(NameReg)
...
const email: joi.StringSchema = joi
  ...
  .required()
  .email()
  .label('您输入的Email地址格式不正确');
const password: joi.StringSchema = joi
  .string()
  .min(6)
  .max(20)
  .regex(PasswordReg, 'password')
  .label('密码不能为空, 长度必须大于6, 小于20, 且同时包括字母和数字');
const avatar: joi.StringSchema = joi.string();
